...
本ドキュメントは、自律型コーディングエージェント (Jules / Devin / Codex / Claude Code / GitHub Copilot / Cursor / Cline / Windsurf / Aider / Sweep / PR-Agent 等) が、公開 OSS リポジトリ [`genzouw/dice-simulator`](https://github.com/genzouw/dice-simulator) で作業し Pull Request を作成するときに **必ず守るべき規範** を定義します。
...
上記をひとつでも満たさないサービスを組み込む PR は **MUST NOT** で、提出された場合は内容の良し悪しにかかわらず自動的にクローズされます。ただし、5章の手続きに従い PR 作成前に Issue でリポジトリオーナー (@genzouw) の明示的な承認を得ている場合はこの限りではありません。
...
- [ ] LLM プロバイダの API キー / 従量課金 API キーを GitHub Secrets に追加していない。`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` 等を `secrets.*` から参照する記述が、新規または変更されたファイルに含まれていない。`OPENAI_API_BASE_URL` など課金可能な OpenAI 互換エンドポイントへの設定変更も対象に含む。
...
本ポリシーから外れる導入を検討したい場合は、PR を作成する **前に** Issue で提案し、リポジトリオーナー (@genzouw) の明示的な承認を **SHOULD** 取得してください。

Issue で明示的な承認を得た例外は、1.1 の自動クローズ規則より **優先** され、承認された内容の範囲内で PR を作成できます。承認を得ずに提出された有料サービス導入 PR は、Issue の有無にかかわらず自動的にクローズされます。
